...
alphabetListUpperCase = list(string.ascii_uppercase)
specialChar = ['~', ':', "'", '+', '[', '\\', '@', '^', '{', '%', '(', '-', '"', '*', '|', ',', '&', '<', '`', '}', '.', '_', '=', ']', '!', '>', ';', '?', '#', '$', ')', '/' ]
    ...
    for x in listInput:
        ...
        if x in alphabetList:
            ...
            if tmp > 25:
                ...
                mesList.append(alphabetList[tmp])
        if x in alphabetListUpperCase:
            tmp = int(alphabetListUpperCase.index(x)) + int(letterShift)
            if tmp > 25:
                tmp = tmp - 26
                mesList.append(alphabetListUpperCase[tmp])
            else:
                mesList.append(alphabetListUpperCase[tmp])
